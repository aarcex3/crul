    ...
    def self.parse(args)
      new.tap do |options|
        ...
        options.parser = OptionParser.parse(args) do |parser|
          ...
          parser.on("-j", "--json", "Format response as JSON") do
            options.format = Format::JSON
          end
          parser.on("-x", "--xml", "Format response as XML") do
            options.format = Format::XML
          end
          parser.on("-p", "--plain", "Format response as plain text") do
          ...
          parser.unknown_args do |unknown_args|
            if unknown_args.empty?
              ...
              uri = URI.parse(unknown_args[0])
              options.url = URI.parse("http://#{unknown_args[0]}") if uri.host.nil?
            end
          end
